...
//    }
    ...
    public ResponseEntity<Object> login(@RequestBody RecruiterLogin loginRequest) throws Exception {
       JobRecruiter recruiter = recruiterService.login(loginRequest.getEmail(), loginRequest.getPassword());
        ...
        if (recruiter!=null) {
        	return createAuthenticationToken(recruiter);
        } else {
            return new ResponseEntity<>("failed", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    private ResponseEntity<Object> createAuthenticationToken(@RequestBody JobRecruiter recruiter) throws Exception {
		// TODO Auto-generated method stub
    	try {
			authenticationManager.authenticate(
					new UsernamePasswordAuthenticationToken(recruiter.getEmail(), recruiter.getPassword())
			);
		}
